...
def draw_board():
    ...
    if selected_square is not None:
        svg = chess.svg.board(board, size=640, fill={square: '#68BC00'})
        png_io = io.BytesIO()
        ...
        for event in pygame.event.get():
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_BACKSPACE:
                    ...
                    print(games_archive,games_archive_index)
